...
import { formItem, label, rules, show } from "@lby/react-form/useForm/decorator"
...
export default () => {
  ...
  function onChange(
    ...
    if (formData.gender === 'male') form.setFormData({ nickName: 'tom👨🏻' }).clearError('nickName')
    if (formData.gender === 'female') form.setFormData({ nickName: 'lily👩🏻' }).clearError('nickName')
    if (formData.gender === 'other') form.setFormData({ nickName: 'siri😅' }).clearError('nickName')
  }
